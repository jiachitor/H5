...
var _ = require('lodash'),
    ...
    config_tpl_sass = require('../config_tpl').sass,
...
gulp.task('sass_to_oneself', function () {
    ...
    _.forEach(config_sass.pages, function (page) {
        ...
        gulp.src(curEntry)
            ...
            .pipe(rename('app.css'))
            .pipe(minifycss())
            .pipe(gulp.dest(config_sass.src + '/'))
...
gulp.task('sass_tpl', function () {

    _.forEach(config_tpl_sass.watch_pages, function (page) {
        var curEntryName = 'app.scss',
            curEntry = config_sass.src + page + '/scss/' + curEntryName;
        if (!fs.existsSync(curEntry)) {//如果当前语言没有入口文件，则使用默认的入口文件
            //curEntry = config_sass.src + curEntryName;
        ...
        gulp.src(curEntry)
            ...
            .pipe(browserSync.reload({stream: true}))
            .pipe(notify({message: 'the sass_tpl to build complete'}));
    });
