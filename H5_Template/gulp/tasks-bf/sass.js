...
gulp.task('sass_to_together', function () {
    var entries = [],
        stream;
    _.forEach(config.pages, function (entry) {
        var curEntryName = 'main.sass',
            curEntry = config.src + entry + '/sass/' + curEntryName;
...
//单个sass文件转换为css文件
gulp.task('sass_to_oneself', function () {

    _.forEach(config.pages, function (page) {
        var curEntryName = 'app.sass',
            curEntry = config.src + page + '/sass/' + curEntryName;
        if (!fs.existsSync(curEntry)) {//如果当前语言没有入口文件，则使用默认的入口文件
            //curEntry = config.src + curEntryName;
        }
        ...
        gulp.src(curEntry)
            ...
            .pipe(rename('app.css'))
            .pipe(minifycss())
            .pipe(gulp.dest(config.src + page))
